...
- **KIS Open API**: `계좌 연결` 화면의 국내주식 보유수량·매입가·평가액·예수금 조회에 이미 사용합니다. `KIS_ENV`, `KIS_APP_KEY`, `KIS_APP_SECRET`, `KIS_CANO`, `KIS_ACNT_PRDT_CD`를 배포 서버의 Streamlit Secrets에 설정합니다. `real`은 실전 계좌, `demo`는 모의 계좌입니다. 조회 버튼을 누를 때만 연결하며 매매 주문은 구현되어 있지 않습니다.
- **[KRX OpenAPI](https://openapi.krx.co.kr/)**: 2024~2025년 월말 종가·시가총액·상장주식수의 공식 출처 후보입니다. `KRX_AUTH_KEY`를 서버 Secrets에 보관합니다. 인증키 발급과 별개로 **유가증권 일별매매정보 및 코스닥 일별매매정보** 각각의 활용 승인 여부를 확인해야 합니다. 현재 앱의 `설정 → 전체 연결 진단`은 유가증권 일별매매정보의 인증·응답을 확인합니다. 코스닥 승인 및 24개월 월말 자료 수집은 아직 구현되지 않아 키만 설정해도 적정주가에 자동 반영되지 않습니다.
- 재무는 DART 연결 기준을 유지하고, KRX 일별 자료의 마지막 거래일을 각 월말로 골라 24개월을 채우는 작업은 별도 구현·검증이 필요합니다. 응답 누락이나 미승인 서비스는 임의 값으로 대체하지 않습니다. 인증키와 계좌번호를 채팅·GitHub에 올리지 마세요.
